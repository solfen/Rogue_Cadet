...
public class ResponsiveGridLayout : MonoBehaviour {
    ...
    public void Resize() {
        ...
        float rowNb = Mathf.Ceil(target.childCount / colNb);
        ...
        float paddingTocenterY = 1 - ( (1 - (totalSpacingSize.y + rowNb * itemSize.y)) / 2);
        ...
        for (int i = 0; i < target.childCount; i++) {
            ...
            item.anchorMin = new Vector2(padding.x + colID * (spacing.x + itemSize.x), paddingTocenterY - (rowID * (spacing.y + itemSize.y) + itemSize.y));
